...
from notifications.telegram.common import send_telegram_message, send_telegram_message_async, ADMIN_CHAT, VIBES_CHAT, Chat


async def notify_user_ban(user, days, reason):
    if user.telegram_id:
...
async def notify_admins_on_ban(user, days, reason):
    ...
    for chat in [ADMIN_CHAT, VIBES_CHAT]:
        await send_telegram_message_async(chat=chat, text=text)
