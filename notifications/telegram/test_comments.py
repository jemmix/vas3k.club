...
class NotifyOnCommentCreatedTest(TestCase):
    ...
    @patch("notifications.telegram.comments.send_telegram_message_async")
    ...
    async def test_notifies_all_comment_subscribers(self, mock_render, mock_send):
        ...
        await PostSubscription.objects.acreate(
            user=self.subscriber_user,
            post=self.post,
            type=PostSubscription.TYPE_ALL_COMMENTS,
        )

        await notify_on_comment_created(comment)
    ...
    async def test_skips_top_level_only_subscribers_for_reply(self, mock_render, mock_send):
        parent_comment = await Comment.objects.acreate(
        ...
        await notify_on_comment_created(reply)
    ...
    async def test_skips_online_channel_for_draft_post(self, mock_render, mock_send):
        draft_post = await Post.objects.acreate(
            author=self.author,
            title="Draft Post",
            text="Draft text",
    ...
    async def test_posts_to_room_chat(self, mock_render, mock_send):
        ...
        room_post = await Post.objects.acreate(
            author=self.author,
            title="Room Post",
            text="Text",
            visibility=Post.VISIBILITY_EVERYWHERE,
    ...
    async def test_respects_muted_author(self, mock_render, mock_send):
        ...
        await UserMuted.objects.acreate(
