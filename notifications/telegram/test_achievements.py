...
def _create_user(telegram_id=None, moderation_status=User.MODERATION_STATUS_APPROVED, **kwargs):
    defaults = dict(
        full_name="Test User",
...
def _create_achievement(code, image="https://example.com/badge.png", custom_message=None):
...
class NotifyAdminsOnAchievementTest(TestCase):
    ...
    async def test_includes_from_user_when_provided(self, mock_send_message):
        ...
        achievement = _create_achievement(code="from_user_test")
        user_achievement = UserAchievement.objects.create(user=user, achievement=achievement)

        notify_admins_on_achievement(user_achievement, from_user=from_user)
    ...
    async def test_handles_no_from_user(self, mock_send_message):
        ...
        notify_admins_on_achievement(user_achievement, from_user=None)
